...
const
  STREAM_FRAME_RATE = 60; (* 25 images/s *)
...
function main(FileName: string; w, h, FrameRate, duration: Integer;
  BkImage: TBitMap; ObjectList: PObjectLI): Integer;
...
function add_stream(ost: POutputStream; oc: PAVFormatContext; codec: PPAVCodec;
  codec_id: TAVCodecID; w, h, FrameRate: Integer): Integer;
...
function alloc_picture(pix_fmt: TAVPixelFormat; width, height: Integer)
  : PAVFrame;
...
function open_video(oc: PAVFormatContext; codec: PAVCodec; ost: POutputStream;
  opt_arg: PAVDictionary): Integer;
...
function fill_rgb24_image(pict: PAVFrame; frame_index, width, heigth: Integer;
  bmp: TBitMap): Integer;
var
  w, h: Integer;
  x, y, i, j: Integer;
...
begin
  ...
  for y := heigth - 1 downto 0 do
    ...
    begin
      PByte(@PAnsiChar(pict.data[0])[y * pict.linesize[0] + (x * 3)])^ :=
        MemBits[j * 4 + 2];
      PByte(@PAnsiChar(pict.data[0])[y * pict.linesize[0] + (x * 3 + 1)])^ :=
        MemBits[(j * 4 + 1)];
      PByte(@PAnsiChar(pict.data[0])[y * pict.linesize[0] + (x * 3 + 2)])^ :=
        MemBits[(j * 4)];
      Inc(j);
    end;
  Result := 0;
...
function get_video_frame(ost: POutputStream; frame: PPAVFrame;
  duration: Integer; bmp: TBitMap): Integer;
var
  c: PAVCodecContext;
  linesize: PInteger;
  rgbframe: PAVFrame;
begin
  ...
  if av_compare_ts(ost.next_pts, ost.enc.time_base, duration, av_make_q(1, 1)) > 0
  then
  begin
    frame^ := nil;
    Result := 0;
  ...
  ost.sws_ctx := sws_getContext(c.width, c.height, AV_PIX_FMT_RGB24, c.width,
    c.height, AV_PIX_FMT_YUV420P, 0, 0, 0, 0);
  sws_scale(ost.sws_ctx, @rgbframe.data[0], @rgbframe.linesize[0], 0, c.height,
...
*)
function write_video_frame(oc: PAVFormatContext; ost: POutputStream;
  duration: Integer; bmp: TBitMap): Integer;
var
  frame: PAVFrame;
begin
...
function main(FileName: string; w, h, FrameRate, duration: Integer;
  BkImage: TBitMap; ObjectList: PObjectLI): Integer;
var
  VideoStream, audio_st: TOutputStream;
  outFmt: PAVOutputFormat;
  outContext: PAVFormatContext;
  audio_codec, VideoCodec: PAVCodec;
  ret: Integer;
  have_video: Integer;
  EncodeVideo: Integer;
  opt: PAVDictionary;
...
begin
  ...
  have_video := 0;
  EncodeVideo := 0;
  avformat_alloc_output_context2(@outContext, nil, 'mpeg',
  ...
  begin
    if add_stream(@VideoStream, outContext, @VideoCodec, outFmt.video_codec, w,
      h, FrameRate) < 0 then
    begin
      Result := 1;
      Exit;
  ...
  begin
    ret := avio_open(@outContext.pb, PAnsiChar(AnsiString(FileName)),
      AVIO_FLAG_WRITE);
    if ret < 0 then
    begin
      Result := 1;
  ...
  begin
    ...
    begin
      ...
      IsEnd := True;
      TMainForm.DrawFrame(ObjectList, buff, IsEnd, Round(CurrTime));
      EncodeVideo := write_video_frame(outContext, @VideoStream,
        duration, buff);
      if EncodeVideo < 0 then
      begin
        Result := 1;
